#!/usr/bin/env node
/**
 * Wave 149 Test Runner — Dark Theme Fix — Wave 148 Screens
 *
 * Runs all wave-specific tests (API + E2E flows) and produces result artifacts.
 *
 * Usage:
 *   node automatron/waves/wave-149/tests/wave-149-runner.cjs [--pre|--post]
 *
...
async function main() {
  ...
  // --- E2E Flows (source-level — no Puppeteer needed for this wave) ---
  if (flows.length > 0) {
    console.log(`  Running ${flows.length} E2E flow(s) (source-level analysis)...`);

    // Wave 149 flows are source-level static analysis, not browser-based.
    // They accept page=null and analyze source files directly.
    const ctx = { login };
    ...
    for (const flowFile of flows) {
      ...
      try {
        ...
        const result = await flowFn(null, config, ctx);
